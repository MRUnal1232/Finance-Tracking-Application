...
const getDB = () => {
  ...
  if (!database) {
    // Try to get database from firebase module
...
const getUserRef = (path: string = '') => {
  const db = getDB();
  if (!db) {
    throw new Error('Database not available during SSR or Firebase not initialized');
  }

  const auth = getAuth();
  const user = auth.currentUser;
  if (!user) {
    throw new Error('No user logged in. Please sign in to access this feature.');
  }
