...
const IncomeList = dynamic(
  // Prefer the named export but fall back to the default export if needed.
  () => import("@/components/income-list").then((mod) => ({ default: mod?.IncomeList ?? mod?.default })),
  {
    loading: () => <LoadingSkeleton type="list" />,
    // Ensure this client-only chunk is not attempted to be serialized by RSC.
    ssr: false
  }
)
